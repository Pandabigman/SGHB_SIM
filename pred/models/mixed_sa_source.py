...
class MixedSASourceModel(BaseModel):
    ...
    def _run_with_csv_data(self, Ne, generations, lambda_val, stochastic, genetic_data,
        ...
        # Use mixed source simulation with caching (provides Ho/He/F in stochastic mode)
        ...
        # Primary source of novel alleles is PAAZA (6/10 birds). KZN and EC wild birds
        # share most alleles with the main wild population so use PAAZA novel pool.
        ...
        # Pool size = PAAZA (captive source); KZN/EC wild birds are part of wild_all
    ...
    def _run_generic(self, Ne, generations, lambda_val, stochastic,
        ...
        # Mixed SA sources: primarily PAAZA captive, use conservative PAAZA-level default.
        if max_novel_alleles is None:
            max_novel_alleles = 1.5  # PAAZA-only surplus (KZN/EC wild overlap with wild_all)
